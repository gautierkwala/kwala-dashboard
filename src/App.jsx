...
function getObjectifs(granularite) {
  ...
  return { ca: OBJ_CA_MENSUEL * factor, rdv: OBJ_RDV_MENSUEL * factor, deals: OBJ_DEALS_MENSUEL * factor };
}
...
// Delta entre valeur courante et précédente, formaté
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-btn { background: none; border: none; padding: 0 9px; height: 100%; cursor: pointer; color: var(--txt2); font-size: 15px; display: flex; align-items: center; transition: background 0.1s; }
  ...
  .pill { padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 500; border: 0.5px solid var(--bdr2); background: transparent; color: var(--txt2); cursor: pointer; white-space: nowrap; transition: all 0.1s; font-family: 'DM Sans', sans-serif; display: flex; align-items: center; gap: 3px; }
  ...
  .kpi { background: var(--surface); border: 0.5px solid var(--bdr); border-radius: 8px; padding: 10px 12px; }
...
// Table deals en cours — tri local uniquement, indépendant des filtres globaux
...
export default function App() {
  ...
  // Stats filtrées par coach pour la période courante
  const stats = useMemo(() => {
    if (!data) return null;
    return (coach !== 'tous' && data[coach]) ? data[coach] : data['tous'];
  }, [data, coach]);

  // Stats période précédente (pas de filtre coach — pas assez de data)
